...
uses
  {$IFDEF FPC}
  {$IFDEF MSWINDOWS}Windows,{$ELSE}WinMem,{$ENDIF}SysUtils, Classes, Connector,
  Notifier, ParseCommon, ParseConsts, Parser, ParseTypes, TextConsts, ValueTypes;
  {$ELSE}
  {$IFDEF DELPHI_XE7}
  WinApi.Windows, System.SysUtils, System.Classes, Connector, Notifier, ParseCommon,
  ParseConsts, Parser, ParseTypes, TextConsts, ValueTypes;
  {$ELSE}
  Windows, SysUtils, Classes, Connector, Notifier, ParseCommon, ParseConsts, Parser,
  ParseTypes, TextConsts, ValueTypes;
  {$ENDIF}
  {$ENDIF}
...
uses
  {$IFDEF FPC}
  Math, Types, FastList, FileUtils, IniFiles, NumberConsts, ParseUtils, StrUtils,
  TextUtils, ThreadUtils, ValueConsts, ValueUtils;
  {$ELSE}
  {$IFDEF DELPHI_XE7}
  System.IniFiles, System.Math, System.Types, FastList, FileUtils, NumberConsts,
  ParseUtils, StrUtils, TextUtils, ThreadUtils, ValueConsts, ValueUtils;
  {$ELSE}
  Math, Types, FastList, FileUtils, IniFiles, NumberConsts, ParseUtils, StrUtils,
  TextUtils, ThreadUtils, ValueConsts, ValueUtils;
  {$ENDIF}
...
begin
  Result := GetDouble(AsValue(Index));
end;
...
begin
  Result := GetExtended(AsValue(Index));
end;
...
begin
  Result := GetInteger(AsValue(Index));
end;
...
begin
  Result := GetLongWord(AsValue(Index));
end;
...
begin
  Result := GetShortint(AsValue(Index));
end;
...
begin
  ...
  if Result < 0 then Result := FList.Add(ItemName);
...
begin
  ...
  begin
    ...
    AItem := Pointer(FList.Objects[Result]);
...
begin
  Result := GetSingle(AsValue(Index));
end;
...
begin
  Result := GetSmallint(AsValue(Index));
end;
...
begin
  if FindParser then
    ...
    begin
      ...
      try
        Result := Parser.ExecuteScript(Script)^;
      finally
        Script := nil;
      end;
    end
...
begin
  ...
  begin
    ...
    try
      ...
      if DeletePrefix(ItemName, ParseCommon.Lock) then
      begin
...
begin
  ...
  if Result then
    case StateType of
      stNecessaryExecute:
        Result := not (ifExecuteOK in AItem.Flags) or (ifPermanentExecute in AItem.Flags) or
          (ifLock in AItem.Flags);
      stPermanentExecute:
        Result := FindParser and not (ifExecuteOK in AItem.Flags) and not (ifLock in AItem.Flags) and
          not Helper.Optimizable(AItem.Item.Script, Parser.FData);
      stPossibleSimplify:
...
begin
  ...
  begin
    ...
    try
      ...
      if Result then
      begin
        System.Reset(FileArray[FileIndex]^);
        while not Eof(FileArray[FileIndex]^) do
        begin
          ReadLn(FileArray[FileIndex]^, S);
...
begin
  ...
  begin
    ...
    begin
      Parser.OptimizeScript(AItem.Item.Script);
      if State[AItem, stPossibleSimplify] then
        ParseCommon.AssignValue(AItem.Item, Parser.ExecuteScript(AItem.Item.Script)^)
      else
        AItem.Item.ItemType := itScript;
...
begin
  FileIndex := CreateFile;
  try
    if OpenFile(FileName, otMake) then
      try
        for I := 0 to FList.Count - 1 do
