...
uses
  {$IFDEF FPC}
  SysUtils, Classes, Contnrs, Connector, Notifier, ParseManager, ParseValueList,
  ParseTypes, Parser, Thread, SyncThread, ValueTypes;
  {$ELSE}
  {$IFDEF DELPHI_XE7}
  System.SysUtils, System.Classes, System.Contnrs, Vcl.Forms, Connector, Notifier,
  ParseManager, ParseValueList, ParseTypes, Parser, Thread, SyncThread, ValueTypes;
  {$ELSE}
  SysUtils, Classes, Contnrs, Forms, Connector, Notifier, ParseManager, ParseValueList,
  ParseTypes, Parser, Thread, SyncThread, ValueTypes;
  {$ENDIF}
  {$ENDIF}
...
procedure TThread.Warmup;
var
  Script: TScript;
begin
  if Available(FCalculator) then
  begin
    FCalculator.Parser.StringToScript(Item[0].Text, Script);
    Script := nil;
  end;
end;
...
begin
  Result := Pointer(NativeInt(AsInt64(Text)));
end;
