...
uses
  {$IFDEF FPC}
  SysUtils, Classes, Connector, FlexibleList, Notifier, ParseCommon, ParseConsts,
  Parser, ParseTypes, TextConsts, Types, ValueTypes;
  {$ELSE}
  {$IFDEF DELPHI_XE7}
  WinApi.Windows, System.SysUtils, System.Classes, BaseTypes, Connector, FlexibleList,
  Notifier, ParseCommon, ParseConsts, Parser, ParseTypes, TextConsts, Types, ValueTypes;
  {$ELSE}
  Windows, SysUtils, Classes, BaseTypes, Connector, FlexibleList, Notifier, ParseCommon,
  ParseConsts, Parser, ParseTypes, TextConsts, Types, ValueTypes;
  {$ENDIF}
  {$ENDIF}
...
type
  ...
  protected
    ...
    function Recurse(const FunctionHandle: NativeInt; const Script: TScript): Boolean; virtual;
...
uses
  {$IFDEF FPC}
  MemoryUtils, ParseUtils, StrUtils, TextUtils, ValueConsts, ValueUtils;
  {$ELSE}
  MemoryUtils, ParseUtils, StrUtils, TextUtils, ValueConsts, ValueUtils;
  {$ENDIF}
    ...
    try
      ...
      try
        ...
        begin
          ...
          List[I] := S +
            {$IFDEF DELPHI_7}List.NameValueSeparator{$ELSE}Equal{$ENDIF} + Trim({$IFDEF DELPHI_7}List.ValueFromIndex[I]{$ELSE}GetValueFromIndex(
              List,
              I
            ) {$ENDIF});
        end;
        ...
        if TryTextToValue(S, Item.Item.Value) then ConvertToVariable(AFunction, @Item.Item.Value)
          ...
          begin
            ...
            if Recurse(AFunction.Handle^, Item.Item.Script) then
              ...
              if FOptimization then Parser.OptimizeScript(Item.Item.Script);
              if Optimal(Item.Item.Script, stScript) then
              begin
                ParseCommon.AssignValue(Item.Item, Parser.ExecuteScript(Item.Item.Script)^);
                ConvertToVariable(AFunction, @Item.Item.Value);
              end
              else begin
                ConvertToFunction(AFunction, @Item.FHandle);
...
function TParseValueList.Custom(const Header: PScriptHeader; const AFunction: PFunction; const AType: PType;
  ...
  begin
    ...
    if Result then
      case Item.Item.ItemType of
        ...
        itScript: Value := Parser.ExecuteScript(Item.Item.Script)^;
      else
        Value := EmptyValue;
      end;
  end
...
function TParseValueList.Error(const Message: string; const Arguments: array of const): Exception;
begin
...
function TParseValueList.Recurse(const FunctionHandle: NativeInt; const Script: TScript): Boolean;
var
  AFunction: PFunction;
...
begin
  if {$IFDEF DELPHI_XE7}System.SysUtils.SameText{$ELSE}SysUtils.SameText{$ENDIF}(Name, FFunctionName) then
