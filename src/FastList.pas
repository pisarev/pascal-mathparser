...
uses
  ...
  {$ELSE}
  {$IFDEF DELPHI_XE7}
  WinApi.Windows, System.Classes, System.Math, System.Types;
  {$ELSE}
  Windows, Classes, Math, Types;
  {$ENDIF}
  {$ENDIF}
...
type
  ...
  private
    ...
    procedure SetIndexCount(const Value: Integer);
  ...
  protected
    ...
    function Add(const TextType: TTextType; const Index: Integer;
      const Data: TTextData): Integer; overload; virtual;
    function IndexOf(const TextType: TTextType; const Data: TTextData; const S: string;
      const KnownIndex: Integer = -1): Integer; overload; virtual;
    function RoughIndexOf(const TextType: TTextType; const Data: TTextData): Integer; overload; virtual;
    function ExactIndexOf(const TextType: TTextType; const TextCode, Index: Integer; const S: string;
      const KnownIndex: Integer = -1): Integer; virtual;
    function Search(const TextType: TTextType; const TextCode: Integer;
      const S: string): Integer; overload; virtual;
    procedure Synchronize(const TextType: TTextType; const Index: Integer;
      const OffsetType: TOffsetType); {$IFDEF DELPHI_XE5}overload;{$ENDIF}virtual;
    {$IFDEF DELPHI_XE5}
    procedure Synchronize(const TypeArray: array of TTextType; const Index: Integer;
      const OffsetType: TOffsetType); overload; virtual;
  ...
  public
    ...
    function Add(const TextType: TTextType; const Index: Integer; const S: string;
      const TextCode: PInteger = nil): Integer; overload; virtual;
    function Delete(const TextType: TTextType; const TextCode, Index: Integer): Boolean; virtual;
    function Put(const TextType: TTextType; const FromText, ToText: string; const KnownIndex: Integer;
      const FromTextCode: PInteger = nil; const ToTextCode: PInteger = nil): Boolean; virtual;
    function IndexOf(const TextType: TTextType; const S: string; out TextCode: Integer;
      const KnownIndex: Integer = -1): Integer; overload; virtual;
    function Find(const TextType: TTextType; const S: string; out TextCode, Index: Integer;
      const KnownIndex: Integer = -1): Boolean; overload; virtual;
    ...
    procedure ResizeIndexArray(const Size: Integer); virtual;
    procedure LoadContent(const Stream: TMemoryStream); virtual;
    procedure SaveContent(const Stream: TMemoryStream); virtual;
    ...
    property IndexCount: Integer read GetIndexCount write SetIndexCount;
  end;
  ...
  TCodeArray = array[TTextType, TIndexFlag] of {$IFDEF DELPHI_10.2}TArray<Integer>{$ELSE}TIntegerDynArray{$ENDIF};
  ...
  protected
    ...
    {$ENDIF}
    procedure SetFlag(const TypeArray: array of TTextType;
      const FlagArray: array of TIndexFlag); overload; virtual;
    function AddCode(const TextType: TTextType; const IndexFlag: TIndexFlag;
      const TextCode: Integer): Integer; overload; virtual;
    procedure AddCode(const TextType: TTextType; const IndexFlag: TIndexFlag;
      const CodeArray: array of Integer); overload; virtual;
  ...
  public
    ...
    function Find(const S: string; var Index: Integer): Boolean; overload; virtual;
    function Find(const TextType: TTextType; const S: string; var Index: Integer): Boolean; overload; virtual;
    function Find(const S: string; const TextType: TTextType = ttNameValue): Boolean; overload; virtual;
    function IndexOf(const S: string): Integer; overload; override;
    function IndexOfName(const Name: string): Integer; override;
    procedure Insert(Index: Integer; const S: string); override;
    procedure InsertObject(Index: Integer; const S: string; AObject: TObject); {$IFNDEF FPC}override;{$ENDIF}
    procedure LoadFromFile(const FileName: string); override;
    procedure LoadFromStream(Stream: TStream); override;
    procedure LoadContent(const Stream: TMemoryStream); virtual;
    procedure LoadContentFromFile(const FileName: string); virtual;
    procedure SaveContent(const Stream: TMemoryStream); virtual;
    procedure SaveContentToFile(const FileName: string); virtual;
...
function SearchCompare(const Target, Value: Pointer; const Index: Integer;
...
function SortCompare(const AIndex, BIndex: Integer; const Target: Pointer;
  const Data: Pointer = nil): TValueRelationship;
procedure SortExchange(const AIndex, BIndex: Integer; const Target, Data: Pointer);
...
function BinarySearch(const Target: TBindArray; const HashCode: Integer): Integer;
begin
  Result := BSearch(Pointer(Target), Low(Target), High(Target), {$IFDEF FPC}@{$ENDIF}SearchCompare, @HashCode);
end;
...
function SortCompare(const AIndex, BIndex: Integer; const Target, Data: Pointer): TValueRelationship;
var
  BindArray: TBindArray absolute Target;
begin
...
begin
  Size := Length(Target);
  Result := MemoryUtils.Delete(Pointer(Target), Index * SizeOf(TBind), SizeOf(TBind), Size * SizeOf(TBind));
...
function THash.Add(const TextType: TTextType; const Index: Integer; const S: string;
  const TextCode: PInteger = nil): Integer;
var
  Data: TTextData;
begin
...
begin
  Result := (Index >= Low(FIndexArray[TextCode].BindArray[TextType])) and
    (Index <= High(FIndexArray[TextCode].BindArray[TextType]));
end;
...
begin
  Result := Check(TextType, TextCode, Index) and
    FastList.Delete(FIndexArray[TextCode].BindArray[TextType], Index);
end;
...
var
  ...
  begin
    Result := ((KnownIndex < 0) or (KnownIndex = FIndexArray[TextCode].BindArray[TextType, I].Index)) and
      Same(Text[TextType, FIndexArray[TextCode].BindArray[TextType, I].Index], S, not FList.CaseSensitive);
  end;
...
function THash.Find(const TextType: TTextType; const S: string; out TextCode, Index: Integer;
  const KnownIndex: Integer): Boolean;
begin
  Index := IndexOf(TextType, S, TextCode, KnownIndex);
  Result := Index >= 0;
end;

function THash.GetIndexCount: Integer;
begin
...
function THash.IndexOf(const TextType: TTextType; const Data: TTextData; const S: string;
  const KnownIndex: Integer): Integer;
begin
  ...
  begin
    ...
    Result := Search(TextType, Data.TextCode, S);
end;

function THash.IndexOf(const TextType: TTextType; const S: string; out TextCode: Integer;
  const KnownIndex: Integer): Integer;
var
  HashCode: Integer;
begin
...
var
  B: TBlobManager;
  Data, Item: PItem;
  I, Size: {$IFDEF CPU64}Int64{$ELSE}Integer{$ENDIF};
  J: TTextType;
begin
  B := TBlobManager.Create(nil);
  try
    B.Open(Stream);
    Item := B.Find('IndexArray.Size');
...
procedure THash.ResizeIndexArray(const Size: Integer);
var
  ASize: Integer;
begin
  ASize := Length(FIndexArray);
  SetLength(FIndexArray, Size);
  Resize(FIndexArray, ASize * SizeOf(TIndex), Size * SizeOf(TIndex));
end;
...
procedure THash.SaveContent(const Stream: TMemoryStream);
var
  B: TBlobManager;
  Data, Size: NativeInt;
  I: Integer;
...
procedure THash.SetIndexCount(const Value: Integer);
begin
...
begin
  case TextType of
    ttName:
      if Value <> '' then
        {$IFDEF FPC}
        FList[Index] := Value + FList.NameValueSeparator + FList.ValueFromIndex[Index];
        {$ELSE}
        FList[Index] := Value +
          {$IFDEF DELPHI_7}FList.NameValueSeparator{$ELSE}Equal{$ENDIF} + {$IFDEF DELPHI_7}FList.ValueFromIndex[Index]{$ELSE}GetValueFromIndex(FList, Index){$ENDIF};
        {$ENDIF}
  else
    FList[Index] := Value;
  end;
...
{$IFDEF DELPHI_XE5}
procedure THash.Synchronize(const TypeArray: array of TTextType; const Index: Integer;
  const OffsetType: TOffsetType);
var
  I: Integer;
begin
...
function TFastList.AddCode(const TextType: TTextType; const IndexFlag: TIndexFlag;
  const TextCode: Integer): Integer;
begin
...
procedure TFastList.AddCode(const TextType: TTextType; const IndexFlag: TIndexFlag;
  const CodeArray: array of Integer);
var
  I: Integer;
begin
...
procedure TFastList.AddStrings(AStrings: TStrings);
begin
  EnterCriticalSection(FLock);
  try
...
begin
  EnterCriticalSection(FLock);
  try
    if Source is TFastList then
    begin
      Stream := TMemoryStream.Create;
      try
        TFastList(Source).SaveContent(Stream);
        Stream.Position := 0;
        LoadContent(Stream);
      finally
        Stream.Free;
  ...
  try
    ...
    for I := Low(TTextType) to High(TTextType) do
      ...
      begin
        ...
        if FHash.Check(I, Item) and (not FHash.Find(I, Item, J, K, Index) or not FHash.Delete(I, J, K)) then
          Reset(I);
...
function TFastList.Find(const TextType: TTextType; const S: string; var Index: Integer): Boolean;
begin
  EnterCriticalSection(FLock);
  try
    Index := IndexOf(TextType, S);
    Result := Index >= 0;
  finally
    LeaveCriticalSection(FLock);
  end;
...
function TFastList.Find(const S: string; const TextType: TTextType): Boolean;
begin
  EnterCriticalSection(FLock);
  try
    Result := IndexOf(TextType, S) >= 0;
  finally
    LeaveCriticalSection(FLock);
  end;
end;
...
begin
  EnterCriticalSection(FLock);
  try
    if (UpdateCount = 0) and (ttNameValue in FIndexTypes) then
      Result := IndexOf(ttNameValue, S)
    else
      if not Sorted then
...
begin
  EnterCriticalSection(FLock);
  try
    if (UpdateCount = 0) and (ttName in FIndexTypes) then
...
procedure TFastList.LoadContent(const Stream: TMemoryStream);

  procedure LoadTextType(const Value: TTextType; Data: PItem);
  var
    Flag: Boolean;
  begin
    if Assigned(Data) then
    begin
      Flag := PBoolean(Data.Stream.Memory)^;
      if Flag then
        Include(FIndexTypes, Value)
      else
        Exclude(FIndexTypes, Value)
    end
    else
      Exclude(FIndexTypes, Value);
  end;
...
var
  ...
  Data: PItem;
begin
  for I := Low(TTextType) to High(TTextType) do Include(FIndexTypes, I);
  BeginUpdate;
  try
    B := TBlobManager.Create(nil);
    try
      B.Open(Stream);
      {$IFDEF DELPHI_10.2}
      Data := B.Find('LineBreak');
      if B.ExportText(Data, S) then LineBreak := S;
      {$ENDIF}
      Data := B.Find('Delimiter');
      if Assigned(Data) then Delimiter := PChar(Data.Stream.Memory)^;
      Data := B.Find('QuoteChar');
      if Assigned(Data) then QuoteChar := PChar(Data.Stream.Memory)^;
      Data := B.Find('NameValueSeparator');
      if Assigned(Data) then NameValueSeparator := PChar(Data.Stream.Memory)^;
      Data := B.Find('CaseSensitive');
      if Assigned(Data) then CaseSensitive := PBoolean(Data.Stream.Memory)^;
      Data := B.Find('Duplicates');
      if B.ExportText(Data, S) then
        Duplicates := TDuplicates(GetEnumValue(TypeInfo(TDuplicates), S));
      Data := B.Find('Sorted');
      if Assigned(Data) then Sorted := PBoolean(Data.Stream.Memory)^;
      Data := B.Find('Text');
      if B.ExportText(Data, S) then Text := S;
      Data := B.Find('Hash');
      if Assigned(Data) then FHash.LoadContent(Data.Stream);
      Data := B.Find('IndexTypes.NameValue');
      LoadTextType(ttNameValue, Data);
      Data := B.Find('IndexTypes.Name');
      LoadTextType(ttName, Data);
    finally
      B.Free;
    end;
    for I := Low(TTextType) to High(TTextType) do FIndexFlags[I] := [];
  finally
    EndUpdate;
  end;
end;

procedure TFastList.LoadContentFromFile(const FileName: string);
var
  Stream: TMemoryStream;
begin
  Stream := TMemoryStream.Create;
...
procedure TFastList.SaveContent(const Stream: TMemoryStream);
var
  B: TBlobManager;
  S: TMemoryStream;
  Flag: Boolean;
begin
  ...
  try
    ...
    FHash.SaveContent(S);
    B.Import(dkData, 'Hash', S);
    Flag := ttNameValue in FIndexTypes;
    B.Import(dkData, 'IndexTypes.NameValue', @Flag, SizeOf(Boolean));
    Flag := ttName in FIndexTypes;
    B.Import(dkData, 'IndexTypes.Name', @Flag, SizeOf(Boolean));
    {$IFDEF DELPHI_10.2}
    B.ImportText('LineBreak', LineBreak);
    {$ENDIF}
    B.ImportText('Delimiter', Delimiter);
    B.ImportText('QuoteChar', QuoteChar);
    B.ImportText('NameValueSeparator', NameValueSeparator);
    Flag := CaseSensitive;
    B.Import(dkData, 'CaseSensitive', @Flag, SizeOf(Boolean));
    B.ImportText('Duplicates', GetEnumName(TypeInfo(TDuplicates), Ord(Duplicates)));
    Flag := Sorted;
    B.Import(dkData, 'Sorted', @Flag, SizeOf(Boolean));
    B.ImportText('Text', Text);
    B.Save(Stream);
  finally
    B.Free;
  end;
...
procedure TFastList.SaveContentToFile(const FileName: string);
var
  Stream: TMemoryStream;
begin
  Stream := TMemoryStream.Create;
...
begin
  for I := Low(TTextType) to High(TTextType) do
    if (I in FIndexTypes) and (FIndexFlags[I] <> []) then
    begin
      if FIndexFlags[I] = [ifDelete] then
        ...
        if FIndexFlags[I] = [ifEnable] then
